...
if (failures.length) {
  ...
  /*
   * ويُتخطّى بعلمٍ صريحٍ لا صامتًا.
   *
   * بيئةُ التطوير هنا محجوبةٌ عن `supabase.co`، فلا تُرسم الصفحاتُ التي
   * تعتمد على القاعدة. ولو جُعل الحارسُ تحذيرًا لَمرّ في السير أيضًا
   * ونُشر كتالوجٌ فارغ. فيبقى مانعًا، ويُرفع بمتغيّرٍ يُكتب باليد —
   * **ولا يُكتب في سير العمل**.
   *
   *     الحارسُ الذي يُتخطّى بالسكوت ليس حارسًا.
   */
  if (process.env.OSOUL_ALLOW_EMPTY === '1') {
    console.error('  ⚠ OSOUL_ALLOW_EMPTY=1 — تُخطِّي الحارسَ عمدًا. لا تضعه في سير العمل.')
  } else {
    process.exit(1)
  }
}
