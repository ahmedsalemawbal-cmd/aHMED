...
console.log('\n═══ نظام اللوحة ═══')
{
  ...
  /* ولا باكتيكَ داخل الأنماط.
   *
   * `PANEL_CSS` قالبٌ نصّيّ يُفتح بباكتيك ويُغلق به. وباكتيكٌ في تعليقٍ
   * داخله يُنهيه في موضعه، فيصير ما بعده شيفرةً لا نصًّا — ويسقط
   * الملفّ برسالةٍ لا تدلّ على السبب («Unexpected identifier»).
   *
   * وقعتُ فيه مرّتين: هنا، وفي مستورد التصاميم قبله.
   *
   *     ما يُغلق النصَّ لا يُكتب داخله.
   */
  /* والحدُّ يُقاس بالمُغلِق لا بآخر باكتيكٍ في الملفّ: بعد الأنماط
     تعليقٌ فيه باكتيكات، فقياسي الأوّل تجاوز المُغلِق إليها وشكا
     من نفسه. والمُغلِقُ باكتيكٌ وحده في سطره. */
  const src2 = fs.readFileSync(R('src/panel.js'), 'utf8')
  const openAt = src2.indexOf('`', src2.indexOf('const PANEL_CSS'))
  const closeAt = src2.indexOf('\n`\n', openAt)
  const inCss = src2.slice(openAt + 1, closeAt)
  const ticks = (inCss.match(/`/g) || []).length
  ok(ticks === 0, 'ولا باكتيكَ داخل الأنماط',
    ticks ? `${ticks} — يُنهي القالبَ في موضعه` : 'نظيف')
}
