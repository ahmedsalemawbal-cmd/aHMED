...
;(function () {
  ...
  /**
   * ما تعرضه اللوحة — يُسأل عنه الخادم في كلّ فتحة.
   *
   * اللوحةُ تعرف **كيف** ترسم زرًّا، ولا تعرف **ما** الأزرار. فيُضيف
   * المالكُ أداةً بصفٍّ في القاعدة فتصل الجميعَ في اللحظة — بلا نسخةٍ
   * جديدةٍ ولا مراجعةِ متجر.
   *
   *     ما يتبدّل يُوصَف. وما يُنفَّذ يُشحن ويُراجَع.
   *
   * والفرقُ عمّن سبقنا أنّ الردّ **بيانٌ يُرسَم**، لا HTML يُنفَّذ.
   * فلو اختُرق خادمُنا لم يزد المهاجمُ على أن يُبدّل اسمَ زرٍّ — ولا
   * يُشغّل سطرًا واحدًا في صفحةٍ المعلّمُ داخلٌ فيها إلى نور.
   */
  function panelInfo() {
    return call('panel')
  }
  ...
  /* ═══════════════════ ما يُصدَّر ═══════════════════ */
    ...
    self.Midad = { API, getKey, setKey, verifyKey, sendTable, readTables, panelInfo }
})()
