...
/**
 * الأرقام الهندية — وهي أرقام العربية في العرض.
 *
 * **ووردبريس لا يحوّلها.** `number_format_i18n` يترجم الفاصلة والعلامة
 * العشرية وحدهما (فيخرج «3٬100»: أرقام لاتينية بفاصلة عربية)، و`wp_date`
 * يترجم أسماء الشهور والأيام ويترك الرقم كما هو. فكل رقم في التطبيق
 * يخرج لاتينيًّا في شاشةٍ كلّها عربية — وهو أوضح فرقٍ بين ما صُمِّم وما
 * يُعرض، ولا يظهر في أي فحص لأن الناتج «صحيح».
 *
 * والتحويل **للعرض وحده**: انظر `sch_localize_output()` وحارسها.
 */
...
/**
 * تفعيل الأرقام الهندية في تطبيق الأسرة وحده.
 *
 * مربوطٌ بالتصيير لا بالإقلاع: الداشبورد وبقيّة الواجهات تبقى على ما
 * هي، فلا تنقلب جداول المالية تحت المحاسب بلا طلب.
 */
function sch_localize_output(): void
{
    add_filter('number_format_i18n', static function ($formatted): string {
        return sch_ar_digits((string) $formatted);
    });

    add_filter('wp_date', static function ($date, $format): string {
        $date = (string) $date;

        /* **الصيغة الآليّة لا تُوطَّن أبدًا.** قيمة `datetime-local` و
           `value` و`datetime` يقرأها المتصفّح لا الإنسان، وأرقامٌ هندية
           فيها تجعله يهمل القيمة بلا رسالة. وكل صيغة آليّة تبدأ بتاريخ
           ISO — فالحارس على الناتج لا على الصيغة، فيصمد لو تغيّرت. */
...
function sch_clock(?string $hm): string
{
    ...
    /*
     * `gmmktime` لا `mktime`، والتنسيق بتوقيت UTC صراحةً.
     *
     * ووردبريس يثبّت توقيت PHP على **UTC**، فـ`mktime` تبني الطابع الزمنيّ
     * على أنه UTC ثم تُعيد `wp_date` عرضه بتوقيت الموقع — **فتكتسب كل
     * ساعةٍ تمرّ من هنا إزاحةَ الموقع**: «07:30» تُطبع «10:30» في السعودية.
     * وهذه الدالّة تُنسّق ساعة الجدار لا لحظةً في الزمن، فلا إزاحة أصلًا.
     */
    return wp_date('H:i', gmmktime((int) $m[1], (int) $m[2], 0, 1, 1, 2000), new DateTimeZone('UTC'));
}
