...
if (!$class) {
    echo '<div class="scha-empty"><strong>' . esc_html__('الفصل غير موجود', 'school-system') . '</strong></div>';
    return;
}
...
?>
<a class="scha-back" href="<?php echo esc_url(SCH_Teacher::url()); ?>">‹ <?php esc_html_e('فصولي', 'school-system'); ?></a>
<h1 class="scha-h1"><?php echo esc_html(SCH_Classes::label($class)); ?></h1>

<?php if ($unmarked !== []) : ?>
    <div class="scha-alert scha-alert--error">
        <strong><?php echo esc_html(sprintf(
            /* translators: %d: عدد الطلاب */
            _n('لم يُرصد دخول %d طالب — راجعه', 'لم يُرصد دخول %d طلاب — راجعهم', count($unmarked), 'school-system'),
            count($unmarked)
        )); ?></strong>
        <span class="scha-card__sub"><?php esc_html_e('المسح هو الأصل وأنت المصحّح: انظر أمامك وحدّد الموجود من الغائب.', 'school-system'); ?></span>
    </div>
<?php endif; ?>
...
<div class="sch-tiles">
    <?php foreach ($roster as $s) :
        ...
        <div class="sch-tile<?php echo $s->att_status === null ? ' sch-tile--wait' : ''; ?>">
            <a class="sch-tile__face" href="<?php echo esc_url(SCH_Teacher::url('student', (int) $s->id)); ?>">
                <?php if ($photo) : ?>
            ...
            <a href="<?php echo esc_url(SCH_Teacher::url('student', (int) $s->id)); ?>">
                <b><?php echo esc_html(mb_substr($name, 0, 18)); ?></b>
            </a>

            <?php if ($s->att_status === null) : ?>
                <span class="sch-tile__acts">
                    <?php foreach (['present' => __('موجود', 'school-system'), 'absent' => __('غائب', 'school-system')] as $slug => $label) : ?>
                        <form method="post">
                            ...
                            <button class="sch-mini sch-mini--<?php echo $slug === 'present' ? 'y' : 'n'; ?>">
            ...
            <?php else : ?>
                <span class="sch-state sch-state--<?php echo esc_attr($s->att_status === 'absent' ? 'no' : 'ok'); ?>">
